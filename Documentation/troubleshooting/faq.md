...
For example, if the Kubernetes version is 1.8.4, the first Tectonic production release including it is labeled `1.8.4-tectonic.1`. A second release including the same 1.8.4 version of Kubernetes would be `1.8.4-tectonic.2`. When the version number of the included Kubernetes advances to 1.8.5, the associated Tectonic version number would be `1.8.5-tectonic.1`.
