...
For example, if the Kubernetes version is 1.7.5, the first Tectonic production release including it is labeled `1.7.9-tectonic.1`. A second release including the same 1.7.5 version of Kubernetes would be `1.7.5-tectonic.2`. When the version number of the included Kubernetes advances to 1.7.6, the associated Tectonic version number would be `1.7.6-tectonic.1`.
